...
//--Q#1-------------------------------------------------------------------------
    ...
    // find the first element in [first,last) that equals value
    {
        while (first != last && (*first != val))
            ++first;        // temp, replace when defining function
...
//------------------------------------------------------------------------------
    ...
    {
            Line(int id, int p, int a) :
                    Shape { id }, perimeter { p }, area { a }
            {
            }
            int calc_perimeter() const override
            {
                return perimeter;
            }
            int calc_area() const override
            {
                return area;
            }
...
//--Q#5-------------------------------------------------------------------------
    ...
    class Range_low_to_high
    {
        ...
        public:
            Range_low_to_high(Val l, Val h)
            {
                low = l;
                high = h;
            }
            bool operator()(Val value) const
            {
...
int main()
{
    ...
    cout << "vector v: ";
    for (const auto& x : v)
        cout << x << " "; // use auto type deduction (x is an int),
    ...
    for (const auto x : vs)
        cout << *x << " "; // use auto type deduction (x is a Shape*),
    ...
    // free all shapes
    for (Shape* pS : vs)
        delete pS; // delete all shape objects, otherwise memory will leak
    ...
    try
    {
        ...
        if (l.size() < v.size())
            throw length_error("target container too small"); // throw exceptoin when
...
// Q#1: What kinds of operations must type InputIterator and type EqualityComparible support such that find is able to work?
// - InputIterator: single pass for operator or algorithms
// - EqualityComparible: compare for iterator.

// Q#5
// Explain what this means in terms of operations copy_if type parameters must support. What
// operations must these types have such that copy_if will work?
// InputIterator: access values but can not assigned
// OutputIterators: assigned values but can not access
// Pred: accept function object as parameter. It take two value and return the result. This is lambda allow using define behavior
